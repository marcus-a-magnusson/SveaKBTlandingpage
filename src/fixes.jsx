...
const SlideThree = ({ onButtonClick }) => {
  ...
  // 1. Separate Refs for Desktop and Mobile to prevent "collision"
  const desktopVideoRef = useRef(null);
  const mobileVideoRef = useRef(null);

  // 2. Separate Play States
  const [isDesktopPlaying, setIsDesktopPlaying] = useState(false);
  const [isMobilePlaying, setIsMobilePlaying] = useState(false);

  // 3. Shared Volume (since a user likely wants the same volume on both)
  const [volume, setVolume] = useState(1);

  // Unified toggle function that takes specific ref/state setters
  const toggleVideo = (ref, setIsPlaying) => {
  ...
  const handleVolumeChange = (e) => {
    ...
    setVolume(newVolume);
    // Update both just in case one is hidden but still exists
    if (desktopVideoRef.current) desktopVideoRef.current.volume = newVolume;
    if (mobileVideoRef.current) mobileVideoRef.current.volume = newVolume;
  };

  // Helper function to render the player with specific props
  const RenderVideoPlayer = (ref, isPlaying, setIsPlaying) => (
    <div className="video-wrapper">
      <video
        ...
        onClick={() => toggleVideo(ref, setIsPlaying)}
      />
      {!isPlaying && (
        <div
